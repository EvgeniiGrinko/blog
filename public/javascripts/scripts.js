        ...
        $.ajax({
            ...
            if (!data.ok) {
              ...
              $(commentForm).prepend('<p class="error">' + data.error + '</p>');
               ...
               var newComment = '<ul><li style="background-color:#ffffe0;"><div class="head"><a href="/users/' + data.login + '">' + data.login + '</a><spam class="date">Только что</spam></div>' + data.body + '</li></ul>';
